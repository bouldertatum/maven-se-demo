/**
 * Maven Custom Action: getRoamingStatus
...
export default async function getRoamingStatus({ props, settings }: Args) {
  ...
  if (!settings.API_BASE_URL || !settings.API_TOKEN) {
    return { ok: false, code: "configuration_error", message: "The roaming system is not configured" };
  }

  const response = await fetch(
    `${settings.API_BASE_URL}/customers/${encodeURIComponent(props.customerId)}/roaming`,
  ...
  return { ok: true, roaming: await response.json() };
}
